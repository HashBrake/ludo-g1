...
int main(int argc, char **argv) {
  ...
  // 执行内置动作，内置动作共有 10 个
  for (int i = 1; i <= 10; i++) {
    if (!dexh15->runAction(device_address, i, false)) {
      std::cerr << "执行内置动作失败，编号为 " << std::to_string(i) << std::endl;
    }
    std::cout << "执行内置动作成功，编号为 " << std::to_string(i) << std::endl;
    std::this_thread::sleep_for(std::chrono::seconds(1));
    // 电机位置回零
    dexh15->setMotorTargetPosition(device_address, zero_positions);
    std::this_thread::sleep_for(std::chrono::seconds(1));
  }
  // 下使能电机
